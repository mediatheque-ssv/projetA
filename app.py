...
st.title("Répartition optimisée (version stable - tous mois)")
...
if uploaded_file:
    ...
    # =====================================================
    # 5️⃣ ALGORITHME DE RÉPARTITION (VERSION STABLE)
    # =====================================================
    if st.button("Lancer la répartition optimisée"):
        ...
        mois_fr = {
            ...
            'février': 2   # Autres variantes possibles
        }
        ...
        def parse_dt(row):
            try:
                ...
                if mois_nom not in mois_fr:
                    st.warning(f"Mois non reconnu : '{mois_nom}' (utilise janvier/février/mars)")
                    return pd.to_datetime("1900-01-01")
        ...
        # Appliquer le parsing et vérifier les dates
        ...
        for _, row in df_sorted.iterrows():
            if row['dt'] != pd.to_datetime("1900-01-01"):
                ...
                st.write(f"{row['Date']} → {row['dt'].strftime('%d/%m/%Y')}")
        st.write(f"Mois détectés : {sorted(mois_presents)}")
        ...
        # Algorithme d'affectation (version stable)
        ...
        # Statistiques et export (identique)
